...
def main():
    ...
    for max_iters in max_iterations:
        ...
        for test in range(5000):
            ...
            area = mandelbrot_area_latin_cube(real_intervals, im_intervals, max_iters, total_area, points)

            endtime = time.time()

            with open("../data/lc_iters.csv", 'a') as resultsfile:
                writer = csv.writer(resultsfile, delimiter=',')
                writer.writerow([max_iters, points, area, endtime - starttime])


@jit(nopython=True, parallel=True)
def mandelbrot_area_latin_cube(real_intervals, im_intervals, maxiter, total_area, N):
