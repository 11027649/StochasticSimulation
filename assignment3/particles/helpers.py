...
class Optimize_Charge():
    ...
    def inter_particle_distance(self, part1, part2):
        distance = math.sqrt((abs(part1.x - part2.x))**2 + (abs(part1.y - part2.y))**2)
    ...
    def mover(self):
        ...
        if random.random() > 0.5:
            if not math.sqrt((unlucky.x + move)**2 + (unlucky.y)**2) > self.radius:
                unlucky.x += move
        else:
            if not math.sqrt((unlucky.x)**2 + (unlucky.y + move)**2) > self.radius:
                unlucky.y += move
    ...
    def simannealing_step(self, cooling_per_step):
        ...
        if new_score <= self.score:
            ...
            # print(self.step, acceptance_chance)
            # accept it depending on the temperature
            if acceptance_chance < random.random():
                self.score = new_score
                print(self.step, difference, acceptance_chance, "temp: ", self.current_temp, "accepted")
            else:
    ...
    def run_sa(self, steps):
        ...
        for self.step in range(steps):
            # print(step, self.score, self.current_temp, self.inter_particle_distance(self.particles[0], self.particles[1]))
